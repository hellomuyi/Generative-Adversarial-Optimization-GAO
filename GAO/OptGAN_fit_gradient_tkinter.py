#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# @Time   : 2020/12/19 19:55
# @File   : OptGAN_fit_gradient_tkinter.py
...
class OptGAN_fit_gradient_tkinter(object):
    """
    ...
    def train(self, epochs, eval_epochs, verbose, root, canvas, ax1, ax2):
        topk = 5
        ...
        for epoch in range(epochs + 1):  # +1使得画图能到10000
            ...
            if verbose and epoch % eval_epochs == 0:
                ...
                ax2.plot_wireframe(self.mesh_xyz[0], self.mesh_xyz[1], self.mesh_xyz[2], alpha=0.9,
                ...
                canvas.draw()
                # plt.pause(0.05)  # 不然画得太快会卡住，例如某一图还没来得及画又画下一图
                root.after(150)
                root.update()   # 强制刷新
